...
try:
    ...
    print(f"Error de importación en pagos_nota_dialog.py: {e}")
...
class PagosNotaDialog(QDialog):
    """
    Ventana para la recepción de pagos y abonos de una Nota de Venta.
    """
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setWindowTitle("Recepción de Pagos y Abonos")
    ...
    def crear_grupo_busqueda(self, parent_layout):
        grupo = QGroupBox("Datos de la Nota")
        grupo.setStyleSheet(GROUP_BOX_STYLE)
        
        layout = QGridLayout()
        layout.setSpacing(10)
        
        self.btn_buscar_nota = QPushButton("Buscar Nota")
        ...
        layout.addWidget(self.crear_label_pago("Cliente:"), 1, 1)
        ...
        self.txt_cliente_nota = QLineEdit()
        self.txt_cliente_nota.setReadOnly(True)
        self.txt_cliente_nota.setStyleSheet(readonly_style)
        layout.addWidget(self.txt_cliente_nota, 1, 2)
    ...
    def crear_grupo_registro_pago(self, parent_layout):
        ...
        self.txt_memo_pago.setPlaceholderText("Ej: Pago de factura, Abono...")
    ...
    def conectar_senales(self):
        self.btn_buscar_nota.clicked.connect(self.abrir_busqueda_notas)
    ...
    def abrir_busqueda_notas(self):
        """Abre el diálogo para buscar y seleccionar una nota."""
    ...
    def cargar_nota(self, nota_dict):
        ...
        self.txt_cliente_nota.setText(nota_dict.get('cliente_nombre', ''))
        ...
        self.cargar_historial_pagos() # <- Esta función ahora debe cargar el ID
    ...
    def eliminar_pago_seleccionado(self):
        ...
        except ValueError as ve: # Errores de lógica de negocio (ej. nota cancelada)
            self.mostrar_mensaje("Error de Validación", str(ve), QMessageBox.Critical)
        except Exception as e:
...
if __name__ == "__main__":
    # Este bloque es para probar el diálogo de forma independiente
    app = QApplication(sys.argv)
    
    # Asegurar que los imports relativos funcionen al ejecutar este archivo directamente
    current_dir = os.path.dirname(os.path.abspath(__file__))
